...
class Logger(object):
    ...
    def log_custom_plot(self, chart_name, plot):
        wandb.log({chart_name: plot})

    def log_pr_curve(self, ground_truth, predictions):
        # PR curve
        wandb.log({
            "pr": wandb.plot.pr_curve(ground_truth, predictions, labels=None, classes_to_plot=None)
        })

    def log_roc_curve(self, ground_truth, predictions):
        # ROC
        wandb.log({
            "roc": wandb.plot.roc_curve(ground_truth, predictions, labels=None, classes_to_plot=None)
        })
